...
- [x] **17. Marketplace checkout / service booking** — backend + frontend
  - Done. In-platform **buy products** + **book services**, paid by card via the platform's Stripe Checkout (funds to SankofaX's account; per client decision). **Backend (marketplace app):** new `Service` (bookable offering, price/duration/virtual), `Order` + `OrderItem` (product purchase, `order_number`, statuses pending→paid→fulfilled/cancelled/refunded), `ServiceBooking` (statuses pending_payment/pending/confirmed/completed/declined/cancelled, `booking_number`). APIs: `POST /api/marketplace/checkout/` (one-seller/one-currency cart → Order + Stripe session, rolls back the order if Stripe fails), `GET/PATCH /api/marketplace/orders/[<no>/]?role=buyer|seller` (seller fulfils/cancels, buyer cancels-if-unpaid), `GET/POST /api/marketplace/bookings/` + `PATCH .../<no>/` (paid → Stripe & auto-confirm on payment; free → pending request the business confirms/declines), public `GET /api/marketplace/services/`. A shared `payments.py` builds the sessions; the existing Stripe webhook now routes `checkout.session.completed` by `metadata.purpose` to fulfil orders/bookings. Admin for Service/Order(+items inline)/Booking; seed adds 3 products, 3 services, a paid order + a free booking request. Verified with a Stripe-mocked end-to-end test (checkout line items/metadata, webhook→paid, seller fulfil, multi-company rejection, paid booking→confirmed, free booking→pending→confirm, past-date guard).
  - **Frontend:** product detail `/marketplace/[slug]` with a **Buy now** card (qty, contact, shipping → Stripe redirect), services list `/marketplace/services` + service detail `/marketplace/services/[slug]` with a **book** card (paid → Stripe, free → request), product cards now open the in-platform detail page ("Shop"), a "Book a service" CTA on the marketplace hero, and dashboard **Orders** + **Bookings** pages (buyer/seller tabs, seller fulfil/confirm/decline/complete, Stripe-success banner) + nav items. Added an `apiError` helper for clean DRF error messages. Verified in the browser (product buy → order created & rolled back on the placeholder Stripe key, free booking request, seller confirm, buyer order list; no console errors).
  - Note: the dev `.env` has **placeholder Stripe keys**, so live card redirects can't complete here — the Stripe-dependent path is proven via the mocked backend test. Swap in real `sk_test_…`/webhook secret to exercise live checkout. Multi-vendor payouts (Stripe Connect) remain a future upgrade if sellers should receive funds directly.
- [x] **18. Story-promotion packages** (paid founder stories/features) — backend + frontend
  - Done. Paid publishing of founder stories / brand features / press releases, tied to subscriptions via a subscriber discount. **Backend (new `promotions` app):** `StoryPackage` (kind, price, `duration_days`, `subscriber_discount_percent`, features) + `StorySubmission` (story content, `reference`, status pending_payment→in_review→published/rejected, `amount`, stripe fields, `published_post` link, `featured_until`, admin feedback). APIs: public `GET /api/promotions/packages/` (prices reflect the caller's active-subscriber discount via `your_price`), `GET/POST /api/promotions/submissions/` (multipart, owner-only for their company → creates submission + Stripe session; rolls back on Stripe failure), `GET /api/promotions/submissions/<ref>/`. Shared Stripe webhook now also routes `purpose='story'` → marks paid → `in_review`. Admin `approve_and_publish` action creates a **featured `BlogPost`** in the right category (founder/brand → Success Stories, press → Diaspora News) with `featured_until`, and `reject` with feedback; `expire_featured_stories` management command un-features after the period. Data migration seeds 3 packages; seed adds a sample in-review submission. Verified with a Stripe-mocked end-to-end test (subscriber discount 99→79.20, submit→pay→webhook→in_review, non-owner rejected, admin publish→featured post visible on blog, reject with note).
  - **Frontend:** `/promote` packages page (subscriber-discount aware) with a submission **modal** (pick business, headline, story body, cover upload, contact → Stripe redirect), dashboard **`/dashboard/promotions`** page (submissions with status chips, "view published story" link, editor feedback on rejection) + nav item, and a "Promote Your Story" footer link. `apiError` helper reused for clean DRF errors. Verified in the browser (packages, modal form + subscriber pricing, submit wiring with graceful placeholder-Stripe error, dashboard in-review/rejected states; no console errors).
  - Note: same **placeholder Stripe keys** limitation as #16/#17 — live redirects can't complete in dev; the Stripe path is proven via the mocked backend test.
...
- **2026-07-27** — Item **#18 (Story-promotion packages)** done and verified. New `promotions` app: `StoryPackage` (3 seeded — founder story / brand feature / press release, with `subscriber_discount_percent`) + `StorySubmission` (paid → in_review → admin publishes a **featured BlogPost** / rejects with feedback). APIs for packages (subscriber-discounted `your_price`) + multipart submission; shared Stripe webhook routes `purpose='story'`; `expire_featured_stories` command. **Frontend:** `/promote` packages + submission modal (Stripe redirect), `/dashboard/promotions` page + nav + footer link. Verified via a Stripe-mocked backend test (discount, submit→pay→publish→blog, reject) and in the browser (packages, modal, dashboard states). Placeholder Stripe keys limit live redirects (as #16/#17).
- **2026-07-27** — Item **#17 (Marketplace checkout / service booking)** done and verified. In-platform **buy products** + **book services** paid by card via the platform Stripe Checkout (client chose platform-account payment). **Backend:** `Service`, `Order`/`OrderItem`, `ServiceBooking` models; `/marketplace/checkout/`, `/marketplace/orders/`, `/marketplace/bookings/`, public `/marketplace/services/`; shared `payments.py` + the existing Stripe webhook now routes by `metadata.purpose`; admin + seed. Verified via a Stripe-mocked end-to-end test. **Frontend:** product detail + Buy now, services list + service detail + booking, dashboard Orders & Bookings (buyer/seller tabs + seller actions), marketplace cards now open in-platform detail, `apiError` helper. Browser-verified (free booking, seller confirm, buyer order list). Dev `.env` has placeholder Stripe keys so live card redirects can't complete here — proven via the mocked test; Stripe Connect (seller payouts) noted as a future upgrade.
